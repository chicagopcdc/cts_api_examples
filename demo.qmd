...
* Searching is supported via POST or GET.  I typically use POST for convenience.    
...
## Trials with EGFR positive as an inclusion or exclusion criteria
...
egfr_pos_trials = {'current_trial_status': 'Active',
        'sites.recruitment_status' : 'ACTIVE',
        'primary_purpose': 'TREATMENT',
        'size':10,
        'from':0,
         'biomarkers.nci_thesaurus_concept_id': ['C134501'],
        'biomarkers.eligibility_criterion':'inclusion'
      #  'include':includes
        }          
r = requests.post('https://clinicaltrialsapi.cancer.gov/api/v2/trials',
                  json = egfr_pos_trials, headers=cts_api_header)

j = r.json() 

mr.JSON(j) 

egfr_df = pd.DataFrame(j['data'])
itables.show(egfr_df, column_filters="header",
  buttons=["pageLength", "copyHtml5", "csvHtml5", "excelHtml5"])
summary_df = egfr_df[['nct_id', 'official_title']]  
itables.show(summary_df, column_filters="header",
  buttons=["pageLength", "copyHtml5", "csvHtml5", "excelHtml5"])
time.sleep(2)
```
